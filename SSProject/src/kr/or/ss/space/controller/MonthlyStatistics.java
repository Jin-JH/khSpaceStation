...
public class MonthlyStatistics extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RequestDispatcher view = request.getRequestDispatcher("/views/partner/content/statisticsMonthly.jsp");
		if(!(psList.isEmpty())) {
			PartnerSpace ps1 = psList.get(0);
			//사업자(공간번호)별 해당 예약 금액(월간)
			ArrayList<SpaceCost> scList = new SpaceService().spaceCostM(ps1.getSpaceNo());
			if(!(scList.isEmpty())) {
