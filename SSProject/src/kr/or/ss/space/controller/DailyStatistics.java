 ...
 */
@WebServlet("/daily.do")
public class DailyStatistics extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RequestDispatcher view = request.getRequestDispatcher("/views/partner/content/statisticsDaily.jsp");
		if(!(psList.isEmpty())) {
			PartnerSpace ps1 = psList.get(0);
			//사업자(공간번호)별 해당 예약 금액
			ArrayList<SpaceCost> scList = new SpaceService().spaceCost(ps1.getSpaceNo());
			if(!(scList.isEmpty())) {
			...
			out.println("<script>location.replace('/allSpaceSelect.do');</script>");
		}
